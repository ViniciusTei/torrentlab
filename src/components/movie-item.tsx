...
function MovieItem({ item }: Props) {
  ...
  return (
    <a key={item.id} href={item?.is_movie ? `/movie/${item?.id}` : `/tvshow/${item.id}`} className="cursor-pointer min-w-[226px] h-[385px] flex-1 bg-gray-700 text-white pt-2 px-0 rounded-sm relative">
