...
function parseFormat(title: string): string {
  ...
  if (/hdts/i.test(title)) return "HDTS";
  if (/HDTS-v2/i.test(title)) return "HDTS v2";
  return "—";
}
...
function TorrentRow({ item, movieId }: { item: JackettItem; movieId: number }) {
  ...
  function handleDownload() {
    startDownload({
      magnet:
        item.magneturl ?? (Array.isArray(item.link) ? item.link[0] : item.link),
      itemId: item.guid,
    ...
    <section id="torrents" className="mb-8">
      ...
      {!isLoading && (!data || data.length === 0) && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Nenhum torrent disponível para essa mídia.
          </AlertDescription>
        </Alert>
      )}
